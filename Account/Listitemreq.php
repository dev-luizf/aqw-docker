...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li>Get Item: <?php echo $_POST['search-term']; ?></li>
		</ul>
		
		<div class="row gutter30">
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Add Item Requeriment</h2>
				</div>
				<form action="additemreq.php" method="POST">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['iid']); ?>" name="iid" class="form-control" placeholder="Item Req ID...">
			...
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Get Item Requeriment</h2>
				</div>
				<form action="" method="post">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['search-term']); ?>" name="search-term" class="form-control" placeholder="Item Req ID...">
						<span class="input-group-btn">
							<button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
						</span>
					</div>
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Req Item ID</b></td>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `items_requirements`";
				...
				$sql = "SELECT * FROM `items_requirements` ORDER BY `ItemID` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['ItemID'];
					$name = $resultado['ReqItemID'];
					$type = $resultado['Quantity'];
					echo "
						<tr>
				...
				if ($total > 0) {
					echo "<br /><br /><center>Other Item Req: ";
					for($n = 1; $n <= $paginas; $n++) {
				...
				$busca_it = mysql_query("SELECT * FROM items_requirements WHERE ((`ItemID` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
						$id = $fetch_it['ItemID'];
						$name = $fetch_it['ReqItemID'];
						$type = $fetch_it['Quantity'];
						echo "
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$name</td>
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>ITEM REQ</td>
				...
				echo "</table>
				<center><a href='Listitemreq.php'><br /><br />Complete list of Item Req</a></center>
				";
			}
		?>
