...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li>Get Mapa: <?php echo $_POST['search-term']; ?></li>
		</ul>
		
		<div class="row gutter30">
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Add Map</h2>
				</div>
				<form action="addmap.php" method="POST">
			...
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Get Map</h2>
				</div>
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Max Players</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Upgrade</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Staff</b></td>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `maps`";
				...
				$sql = "SELECT * FROM `maps` ORDER BY `id` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['id'];
					$name = $resultado['Name'];
					$player = $resultado['MaxPlayers'];
					$upg = $resultado['Upgrade'];
					...
					echo "
						<tr>
							...
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>
								<a href='addmap.php?edit=$id'><img src='template/edit.jpg' width='16px' alt='Função Editar'></a>
							</td>
						</tr>
					";
				}
				...
				$busca_it = mysql_query("SELECT * FROM maps WHERE ((`Name` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
						$id = $fetch_it['id'];
						$name = $fetch_it['Name'];
						$player = $fetch_it['MaxPlayers'];
						$upg = $fetch_it['Upgrade'];
						$staff = $fetch_it['Staff'];
						echo "
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$name</td>
				...
				echo "</table>
				<center><a href='Listmap.php'><br /><br />Complete list of Maps</a></center>
				";
			}
		?>
