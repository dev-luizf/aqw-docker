...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li>Get Map Monster: <?php echo $_POST['search-term']; ?></li>
		</ul>
		
		<div class="row gutter30">
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Add Map Monster</h2>
				</div>
				<form action="addmapmon.php" method="POST">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['add-term']); ?>" name="add-term" class="form-control" placeholder="Map ID...">
			...
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Get Map Monster</h2>
				</div>
				<form action="" method="post">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['search-term']); ?>" name="search-term" class="form-control" placeholder="Map ID...">
						<span class="input-group-btn">
							<button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
						</span>
					</div>
		...
		<table align="center">
		<tr>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Map ID</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Monster ID</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Monster Map ID</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Frame</b></td>
		</tr>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `maps_monsters`";
				...
				$sql = "SELECT * FROM `maps_monsters` ORDER BY `MapID` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['MapID'];
					$name = $resultado['MonsterID'];
					$player = $resultado['MonMapID'];
					$upg = $resultado['Frame'];
					echo "
						<tr>
							...
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$player</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$upg</td>
						</tr>
					";
				}
				...
				if ($total > 0) {
					echo "<br /><br /><center>Other Map Monsters: ";
					for($n = 1; $n <= $paginas; $n++) {
				...
				$busca_it = mysql_query("SELECT * FROM maps_monsters WHERE ((`MapID` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
						$id = $fetch_it['MapID'];
						$name = $fetch_it['MonsterID'];
						$player = $fetch_it['MonMapID'];
						$upg = $fetch_it['Frame'];
						echo "
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>MAP MONSTER</td>
				...
				echo "</table>
				<center><a href='Listmapmon.php'><br /><br />Complete list of Map Monsters</a></center>
				";
			}
		?>
