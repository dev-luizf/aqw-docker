...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li>Get Monster: <?php echo $_POST['search-term']; ?></li>
		</ul>
		
		<div class="row gutter30">
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Add Monster</h2>
				</div>
				<form action="addmon.php" method="POST">
			...
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Get Monster</h2>
				</div>
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Health</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Gold</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Xp</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Class Rep</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>DPS</b></td>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT * FROM `monsters` ORDER BY `id` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);
				...
				while ($resultado = mysql_fetch_assoc($query)) {
					...
					$gold = $resultado['Gold'];
					$xp = $resultado['Experience'];
					$rep = $resultado['Reputation'];
					$dps = $resultado['DPS'];
					echo "
						<tr>
							...
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$level</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$health</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$gold</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$xp</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$rep</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$dps</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>
								<a href='addmon.php?edit=$id'><img src='template/edit.jpg' width='16px' alt='Função Editar'></a>
							</td>
						</tr>
					";
				}
				...
				$busca_it = mysql_query("SELECT * FROM monsters WHERE ((`Name` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
					$id = $fetch_it['id'];
					$name = $fetch_it['Name'];
					$level = $fetch_it['Level'];
					$health = $fetch_it['Health'];
					$gold = $fetch_it['Gold'];
					$xp = $fetch_it['Experience'];
					$rep = $fetch_it['Reputation'];
					$dps = $fetch_it['DPS'];
						echo "
							<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
				...
				echo "</table>
				<center><a href='Listmon.php'><br /><br />Complete list of Monsters</a></center>
				";
			}
		?>
