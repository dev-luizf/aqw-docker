...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li>Get Monster Drop: <?php echo $_POST['search-term']; ?></li>
		</ul>
		
		<div class="row gutter30">
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Add Monster Drop</h2>
				</div>
				<form action="addmondrop.php" method="POST">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['add-term']); ?>" name="add-term" class="form-control" placeholder="Monster ID...">
			...
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Get Monster Drop</h2>
				</div>
				<form action="" method="post">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['search-term']); ?>" name="search-term" class="form-control" placeholder="Monster ID...">
						<span class="input-group-btn">
							<button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
						</span>
					</div>
		...
		<table align="center">
		<tr>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Mon ID</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Item ID</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Chance</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Quantity</b></td>
		</tr>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `monsters_drops`";
				...
				$sql = "SELECT * FROM `monsters_drops` ORDER BY `MonsterID` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['MonsterID'];
					$itid = $resultado['ItemID'];
					$chance = $resultado['Chance'];
					$quant = $resultado['Quantity'];
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$itid</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$chance</td>
				...
				$busca_it = mysql_query("SELECT * FROM monsters_drops WHERE ((`MonsterID` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
					$id = $fetch_it['MonsterID'];
					$itid = $fetch_it['ItemID'];
					$chance = $fetch_it['Chance'];
					$quant = $fetch_it['Quantity'];
						echo "
							<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$itid</td>
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>MONSTER DROP</td>
				...
				echo "</table>
				<center><a href='Listmondrop.php'><br /><br />Complete list of Monsters Drops</a></center>
				";
			}
		?>
