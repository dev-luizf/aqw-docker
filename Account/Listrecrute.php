	...
	<body>
		<div id="tudo">
			...
			<div id="conteudo" class="shadow">
				...
				<p><center>Only Staff have access to the Panel of Recrute</center></p>
...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Player Name</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Email</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Specialty</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Contents</b></td>
		</tr>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `recrute`";
				...
				$sql = "SELECT * FROM `recrute` ORDER BY `id` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['id'];
					$name = $resultado['nome'];
					$type = $resultado['email'];
					$esp = $resultado['especialidade'];
					$cont = $resultado['conteudo'];
					echo "
						<tr>
							...
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$esp</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$cont</td>
						</tr>
					";
				}
				...
				if ($total > 0) {
					echo "<br /><br /><center>Other Recrutes: ";
					for($n = 1; $n <= $paginas; $n++) {
				...
				$busca_it = mysql_query("SELECT * FROM recrute WHERE (('%".$busca_prot."%'))");
				...
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
						...
						$name = $fetch_it['nome'];
						$type = $fetch_it['email'];
						$esp = $fetch_it['especialidade'];
						$cont = $fetch_it['conteudo'];
						echo "
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$name</td>
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>RECRUTE</td>
				...
				echo "</table>
				<center><a href='Listrecrute.php'><br /><br />Complete list of Recrutes</a></center>
				";
			}
		?>
