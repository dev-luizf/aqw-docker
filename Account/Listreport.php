	...
	<body>
		<div id="tudo">
			...
			<div id="conteudo" class="shadow">
				...
				<p><center>Only Staff have access to the Panel of Reports</center></p>
...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Player Reported</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Reason</b></td>
		</tr>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `users_reports`";
				...
				$sql = "SELECT * FROM `users_reports` ORDER BY `id` ASC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['id'];
					$name = $resultado['TargetName'];
				...
				if ($total > 0) {
					echo "<br /><br /><center>Other Reports: ";
					for($n = 1; $n <= $paginas; $n++) {
				...
				$busca_it = mysql_query("SELECT * FROM users_reports WHERE (('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
						$id = $fetch_it['id'];
						$name = $fetch_it['TargetName'];
						$type = $fetch_it['Description'];
						echo "
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$name</td>
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>REPORT</td>
				...
				echo "</table>
				<center><a href='Listreport.php'><br /><br />Complete list of Reports</a></center>
				";
			}
		?>
