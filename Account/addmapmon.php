	...
	<body>
		<div id="tudo">
			...
			<div id="conteudo" class="shadow">
				...
				<p><center>Only Administrators have access to the Panel of Map Monsters</center></p>
...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li><?php if(!(isset($_GET['edit']))){ ?>Add Map Monster<?php }else{ ?>Edit Map Monster<?php } ?></li>
		</ul>
		
		<div class="row gutter30">
		<?php if(!(isset($_GET['edit']))){ ?>
			<div style='padding: 10px 50px 10px 50px;'>
			<h2>Add Map Monster</h2>
				<br />
				<?php
					if(isset($_POST['add'])){
						$mapid = addslashes($_POST['add-term']);
						$monsterid = addslashes($_POST['monid']);
						$mapmonid = addslashes($_POST['mapmid']);
						$frame = addslashes($_POST['fra']);
						
							if(mysql_query("INSERT INTO maps_monsters (`MapID`, `MonsterID`, `MonMapID`, `Frame`) VALUES ('$mapid', '$monsterid', '$mapmonid', '$frame')")){
								
							mysql_query("INSERT INTO admin_logs (`staff`, `info`, `ip`) VALUES ('$user', 'Added the map monster: $mapid', '$ip')");
								
								echo "<b style='color: green'>Map Monster Add with Sucess.</b>";
							}else{
			...
			<form action="" method="POST">
				<table>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Map ID <font color="red" style="font-size: 10px;">(Only Numbers)</font>: </td><td><input type="text" name="add-term" value="<?php echo $_POST['add-term']; ?>"></td>
					</tr>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Monster ID: </td><td><input type="text" name="monid" value="<?php echo $_POST['monid']; ?>"></td>
					</tr>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Monster Map ID: </td><td><input type="text" name="mapmid" value="<?php echo $_POST['mapmid']; ?>"></td>
					</tr>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Frame: </td><td><input type="text" name="fra" value="<?php echo $_POST['fra']; ?>"></td>
					</tr>
					<tr><td style="border: 1px #000; padding: 10px 50px 10px 50px;"></td><td><input type="submit" name="add" value="Add Map Monster"></td></tr>
				</table>
			</form>
		<?php }else{ ?>
			<div style='padding: 10px 50px 10px 50px;'>
				<h2>Editar Mapa Monster</h2>
				<?php
					if(isset($_POST['edd'])){
						$ed_id = addslashes($_POST['ed_id']);
						$ed_monster = addslashes($_POST['ed_monid']);
						$ed_mapmonster = addslashes($_POST['ed_mapmonid']);
						$ed_frame = addslashes($_POST['ed_fra']);
						
							if(mysql_query("UPDATE maps_monsters SET `MonsterID`='$ed_monster', `MonMapID`='$ed_mapmonster' WHERE `MapID`='$ed_id'")){
								
							mysql_query("INSERT INTO admin_logs (`staff`, `info`, `ip`) VALUES ('$user', 'Edited the Map Monster: $ed_id', '$ip')");
								...
								echo "<div class='alert alert-success fade in block-inner'>
									...
									setTimeout(function () { location.href = 'Listmapmon.php';
									}, 5000);
									</script>
									</div>";
							}else{
								echo "<b style='color: red'>Todos os Campos foram preenchidos corretamente, mas ocorreu um erro ao enviar ao banco de dados</b><br /><br />";
							}
						}
				...
				<?php
					...
					$busca_edit = mysql_query("SELECT * FROM maps_monsters WHERE MapID=$edit");
					$conta_edit = mysql_num_rows($busca_edit);
					if($conta_edit > 0){
						$fetch_edit = mysql_fetch_array($busca_edit);
						$edit_id = $fetch_edit['MapID'];
						$edit_monid = $fetch_edit['MonsterID'];
						$edit_mapmonid = $fetch_edit['MonMapID'];
						$edit_fra = $fetch_edit['Frame'];
				?>
					<form action="" method="POST">
						<table>
							<tr>
							...
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Monster ID: </td><td><input type="text" name="ed_monid" value="<?php echo $edit_monid; ?>"></td>
							</tr>
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Map Monster ID: </td><td><input type="text" name="ed_mapmonid" value="<?php echo $edit_mapmonid; ?>"></td>
							</tr>
							<tr>
								<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Frame: </td><td><input type="text" name="ed_fra" value="<?php echo $edit_fra; ?>"></td>
							</tr>
							<tr><td style="border: 1px #000; padding: 10px 50px 10px 50px;"></td><td><input type="hidden" name="ed_id" value="<?php echo $edit_id; ?>"><input type="submit" name="edd" value="Editar Map Monster"></td></tr>
						</table>
					</form>
			<?php
				}else{
					echo "<b>There's no Map Monster with that ID</b>";
				}
			?>
