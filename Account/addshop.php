	...
	<body>
		<div id="tudo">
			...
			<div id="conteudo" class="shadow">
				...
				<p><center>Only Staff have access to the Panel of Shops</center></p>
	...
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<?php if(!(isset($_GET['edit']))){ ?>
			...
			<h2>Add Shop</h2>
				...
				<?php
					if(isset($_POST['add'])){
						...
						$busca_it_add = mysql_query("SELECT Name FROM shops WHERE id='$id'");
						...
						if(empty($id) || empty($nome) || ($id <= 0)){
							...
							echo "<b style='color: red'>There is already a Shop with that ID</b>";
						}else{
							if(mysql_query("INSERT INTO shops (`id`, `Name`, `House`, `Upgrade`, `Staff`, `Limited`) VALUES ('$id', '$nome', '$house', '$upgrade', '$upgrade', '$limited')")){
								
							mysql_query("INSERT INTO admin_logs (`staff`, `info`, `ip`) VALUES ('$user', 'Added the shop: $nome', '$ip')");
								
								echo "<b style='color: green'>Shop Add with Sucess. Shop ID: $id</b>";
							}else{
			...
			<form action="" method="POST">
				<table>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Shop ID <font color="red" style="font-size: 10px;">(Only Numbers)</font>: </td><td><input type="text" name="id" value="<?php echo $_POST['id']; ?>" maxlength="8"></td>
					</tr>
					<tr>
		...
		<?php }else{ ?>
			<div style='padding: 10px 50px 10px 50px;'>
				...
				<?php
					if(isset($_POST['edd'])){
						...
						$ed_limited = addslashes($_POST['ed_limited']);
							...
							if(mysql_query("UPDATE shops SET Name='$ed_name', House='$ed_house', Upgrade='$ed_upgrade', Staff='$ed_staff', Limited='$ed_limited' WHERE id='$ed_id'")){
								
							mysql_query("INSERT INTO admin_logs (`staff`, `info`, `ip`) VALUES ('$user', 'Edited the shop: $ed_id', '$ip')");
								...
								echo "<div class='alert alert-success fade in block-inner'>
									...
									setTimeout(function () { location.href = 'Listshop.php';
									}, 5000);
									</script>
									</div>";
							}else{
				...
				<?php
					...
					$busca_edit = mysql_query("SELECT * FROM shops WHERE id=$edit");
			...
			<?php
					...
					echo "<b>There's no Shop with that ID</b>";
				}
			?>
