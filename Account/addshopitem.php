	...
	<body>
		<div id="tudo">
			...
			<div id="conteudo" class="shadow">
				...
				<p><center>Only Staff have access to the Panel of Items Shop</center></p>
...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<ul class="breadcrumb breadcrumb-top">
			...
			<li><?php if(!(isset($_GET['edit']))){ ?>Add Shop Item<?php }else{ ?>Edit Shop Item<?php } ?></li>
		</ul>
		...
		<?php if(!(isset($_GET['edit']))){ ?>
			...
			<h2>Add Shop Item</h2>
				...
				<?php
					if(isset($_POST['add'])){
						...
						$shopid = addslashes($_POST['sid']);
						$itemid = addslashes($_POST['itid']);
						$quant = addslashes($_POST['qua']);
						
						$busca_it_add = mysql_query("SELECT shops_items WHERE id='$id'");
						...
						if(empty($id) || empty($shopid)){
							...
							if(mysql_query("INSERT INTO shops_items (`id`, `ShopID`, `ItemID`, `QuantityRemain`) VALUES ('$id', '$shopid', '$itemid', '$quant')")){
								
							mysql_query("INSERT INTO admin_logs (`staff`, `info`, `ip`) VALUES ('$user', 'Added the shop item: $id', '$ip')");
								
								echo "<b style='color: green'>Shop Item Add with Sucess.Shop Item ID: $id</b>";
							}else{
			...
			<form action="" method="POST">
				<table>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">ID <font color="red" style="font-size: 10px;">(Only Numbers)</font>: </td><td><input type="text" name="id" value="<?php echo $_POST['id']; ?>" maxlength="8"></td>
					</tr>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Shop ID <font color="red" style="font-size: 10px;">(Only Numbers)</font>: </td><td><input type="text" name="sid" value="<?php echo $_POST['sid']; ?>"></td>
					</tr>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Item ID <font color="red" style="font-size: 10px;">(Only Numbers)</font>: </td><td><input type="text" name="itid" value="<?php echo $_POST['itid']; ?>"></td>
					</tr>
					<tr>
						<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="right">Quantity Remain <font color="red" style="font-size: 10px;">(Only Numbers)</font>: </td><td><input type="text" name="qua" value="<?php echo $_POST['qua']; ?>"></td>
					</tr>
					<tr><td style="border: 1px #000; padding: 10px 50px 10px 50px;"></td><td><input type="submit" name="add" value="Add Shop Item"></td></tr>
				</table>
			</form>
		<?php }else{ ?>
			<div style='padding: 10px 50px 10px 50px;'>
			...
			<?php
					...
					echo "<b>There's no Shop Item with that ID</b>";
				}
			?>
