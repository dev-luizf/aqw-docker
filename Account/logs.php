	...
	<body>
		<div id="tudo">
			...
			<div id="conteudo" class="shadow">
				...
				<p><center>Only Administrators have access to the Panel of Logs</center></p>
...
<div id="page-container" class="full-width">
	<div id="fx-container" class="fx-opacity"><div id="page-content" class="block">
		...
		<div class="row gutter30">
			<div class="block full">
				<div class="block-title">
					<h2><i class="fa fa-search"></i> Get a particular Log Staff</h2>
				</div>
				<form action="logs.php" method="post">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['search-term']); ?>" name="search-term" class="form-control" placeholder="Staff Name...">
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>User</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Info</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>IP</b></td>
		</tr>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT * FROM `admin_logs` ORDER BY `id` DESC LIMIT ".$inicio.", ".$_BS['PorPagina'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['id'];
					$name = $resultado['staff'];
					$info = $resultado['info'];
					$ip = $resultado['ip'];
					echo "
						<tr>
							...
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$info</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$ip</td>
						</tr>
					";
				}
				...
				$busca_it = mysql_query("SELECT * FROM admin_logs WHERE ((`Staff` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%')) ORDER BY `id` DESC");
				...
				if($conta_it > 0){
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>LOG</td>
				...
				echo "</table>
				<center><a href='logs.php'><br /><br />Complete list of Logs</a></center>
				";
			}
		?>
