...
class DefaultCodeDefinitionSet implements CodeDefinitionSet
{
    ...
    public function __construct()
    {
        ...
        $builder->setReplacementText('<span style="text-decoration:line-through;">{param}</span>');
        array_push($this->definitions, $builder->build());

        /* [center] center tag */
        $builder->setTagName('center');
        $builder->setReplacementText('<div style="text-align: center;">{param}</div>');
        array_push($this->definitions, $builder->build());
		
        /* [right] right tag */
        $builder->setTagName('right');
        $builder->setReplacementText('<div style="text-align: right;">{param}</div>');
        array_push($this->definitions, $builder->build());
		
        $sizeValidator = new \JBBCode\validators\sizeValidator();
		
        /* [size] size tag */
        $builder->setTagName('size');
        $builder->setReplacementText('<span style="font-size: {option}%;">{param}</span>');
        $builder->setParseContent(false);
        $builder->setBodyValidator($sizeValidator);
        array_push($this->definitions, $builder->build());
