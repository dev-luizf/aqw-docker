...
SFS_ADMIN_PASSWORD="${SFS_ADMIN_PASSWORD:?SFS_ADMIN_PASSWORD is required}"
SFS_ADMIN_ALLOWED_ADDRESS="${SFS_ADMIN_ALLOWED_ADDRESS:-127.0.0.1}"

echo "Waiting for MySQL at ${MYSQL_HOST}:${MYSQL_PORT}..."
i=0
until MYSQL_PWD="$MYSQL_PASSWORD" mysqladmin ping -h"$MYSQL_HOST" -P"$MYSQL_PORT" -u"$MYSQL_USER" --silent 2>/dev/null; do
  i=$((i + 1))
  if [ "$i" -ge 60 ]; then
...
export GAME_PORT SERVER_NAME SFS_ADMIN_USER SFS_ADMIN_PASSWORD SFS_ADMIN_ALLOWED_ADDRESS
...
envsubst '${GAME_PORT} ${MYSQL_HOST} ${MYSQL_PORT} ${MYSQL_DATABASE} ${MYSQL_USER} ${MYSQL_PASSWORD_XML} ${SFS_ADMIN_USER} ${SFS_ADMIN_PASSWORD_XML} ${SFS_ADMIN_ALLOWED_ADDRESS}' \
